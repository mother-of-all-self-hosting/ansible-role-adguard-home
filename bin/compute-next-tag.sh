...
#
# Tags look like `v<AdGuard Home version>-<release>`:
#
# - if defaults/main.yml points at an AdGuard Home version that has never been
#   released, the release counter restarts at 0 (`v0.107.79-0`)
# - otherwise the counter is incremented (`v0.107.79-1`), but only if something
...
version="$(sed -nE 's|^adguard_home_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the AdGuard Home version from $defaults_path"
	exit 1
fi

# The version values carry a leading `v` (e.g. `v0.107.79`), and so do the tags
# (`v0.107.79-0`). Stripping any `v` before prepending one keeps this correct
